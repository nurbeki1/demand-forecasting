...
function MetricChange({ label, oldVal, newVal, higherIsBetter = false }) {
  ...
  const color =
    Math.abs(diff) < 0.001 ? "var(--text-tertiary)" : improved ? "var(--success)" : "var(--error)";
  const arrow = diff > 0 ? "+" : "";
...
export default function ModelComparePanel() {
  ...
  return (
    <>
      ...
      {loading && (
        <div className="hint">
          {t("compare.trainingHint", "Training new model for comparison... This may take a few seconds.")}
        </div>
      )}
      ...
      {result && (
        <>
          ...
          <div className="compare-actions">
            <button className="btn compare-accept-btn" onClick={handleAccept} disabled={accepting}>
              {accepting ? t("common.loading") : t("compare.acceptNew", "Accept New Model")}
            </button>
            <button className="btn compare-keep-btn" onClick={() => setResult(null)}>
